...
import { useActiveAccount, useActiveWallet, useWalletBalance, ConnectButton } from "thirdweb/react"
import { Bot, Send, Wallet, AlertTriangle, CheckCircle } from "lucide-react"
...
export default function ShadowAgentCore({ client }: ShadowAgentCoreProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Hello! I'm Shadow Agent, your advanced AI assistant for the Diamondz Shadow ecosystem. I can help you with questions, wallet searches, token information, and onchain interactions. How can I assist you today?",
      sender: "agent",
      timestamp: new Date(),
    },
  ])
  ...
  const handleExecuteBundle = async () => {
    if (!pendingBundle || !wallet) {
      setError("Please connect your wallet to execute onchain operations.")
      return
    }
    ...
    try {
      ...
      let result
      if (wallet.sendBatchTransactions) {
        result = await wallet.sendBatchTransactions({
          transactions: pendingBundle.calls,
        })
      } else if (wallet.sendTransaction) {
        const results = []
        for (const call of pendingBundle.calls) {
        ...
        result = { transactionHash: results[0]?.hash || "pending" }
      } else {
    ...
    <Card className="bg-gray-900/50 border-gray-800">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-purple-400">
          ...
          Shadow Agent - Blockchain AI Assistant
          {account?.address && (
      ...
      <CardContent className="space-y-4">
        {!account?.address && (
          <div className="text-center">
            ...
            <p className="text-gray-500 text-xs mt-1">Optional: Connect for onchain operations</p>
          </div>
        )}
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {!wallet ? "Connect Wallet to Execute" : "Approve & Execute"}
                </Button>
        ...
        <div className="flex gap-2">
          <Input
            ...
            placeholder="Ask Shadow Agent about transfers, governance, wallet searches, or any blockchain question..."
